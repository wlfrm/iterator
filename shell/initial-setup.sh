...
if [[ ! -f /.puphpet-stuff/initial-setup-resolv-conf ]]; then
    echo "Copy resolv.conf from host-machine"
    cp -f /tmp/mounted/etc/resolv.conf /etc/resolv.conf
    touch /.puphpet-stuff/initial-setup-resolv-conf
    echo "Finished Copy resolv.conf from host-machine"
fi
